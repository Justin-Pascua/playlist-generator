...
class Playlist(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key = True)
    playlist_title: Mapped[str] = mapped_column(String)
    link: Mapped[str] = mapped_column(unique = True, nullable = False)
    user_id: Mapped[int] = mapped_column(nullable = False)
    created_at: Mapped[str] = mapped_column(nullable = False)       # change this to a suitable datetime type later
...
class SongLink(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key = True)
    song_title: Mapped[str] = mapped_column(unique = True, nullable = False)
